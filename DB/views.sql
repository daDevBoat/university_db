...
/* The first query for the higher grade part */
...
/* The second query for the higher grade part */
...
ORDER BY f.instance_id ASC;


/* The third query for the higher grade part add index on the full_name , but with the index added below */
DROP MATERIALIZED VIEW IF EXISTS teacher_load_per_period;
CREATE MATERIALIZED VIEW teacher_load_per_period AS
SELECT 
...
/* The fourth query for the higher grade part */
...
FROM (
    SELECT l.course_code, i.instance_id, ROUND(SUM(DISTINCT pa.planned_hours * ta.factor)::numeric, 2) AS total_planned_hours, ROUND(SUM(epa.allocated_hours * ta.factor)::numeric, 2) AS total_allocated_hours
    FROM course_instance i
    JOIN planned_activity pa ON pa.instance_id = i.instance_id
    JOIN employee_planned_activity epa ON epa.planned_activity_id = pa.planned_activity_id
    JOIN teaching_activity ta ON ta.teaching_activity_id = pa.teaching_activity_id
...
/* The fifth query for the higher grade part */
